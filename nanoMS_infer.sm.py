...
def preprocess_data(df: pd.DataFrame, scaler, feature_names: List[str]):
	...
	if invalid_mask.any():
		...
		read_name = read_name[~invalid_mask]

	if X.empty:
		raise ValueError("All samples are invalid and cannot be predicted")
	X_scaled = scaler.transform(X)
	print("Preprocessing completed")
	return X_scaled, contig.reset_index(drop=True), position.reset_index(drop=True), read_name.reset_index(drop=True) 
		
def main():
	parser = argparse.ArgumentParser(description="nanoMS single-molecule predictor")
	...
	parser.add_argument("--gpu_id", type=str, default=None, help='omma-separated list of GPU IDs to use (e.g., "0,1,2")')
	...
	X_test, contigs, positions, read_names = preprocess_data(df_test, scaler, feature_names)
	test_loader = DataLoader(M6ADataset(X_test), batch_size=2048, shuffle=False, num_workers=4)
	...
	if args.gpu_id:
			nn_model = torch.nn.DataParallel(nn_model, device_ids=device_ids).eval()
