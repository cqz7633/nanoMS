...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Merge entries at the single-molecule level and perform data cleaning.')
    ...
    parser.add_argument('-c','--chunk_size', type=int, default=100000, help='Number of rows per chunk')
    return parser.parse_args()

def filter_and_average_vectorized(values, std_multiplier=3):
    """Vectorized filtering and averaging function"""
    if len(values) == 1:
        return values[0]
...
def process_chunk_vectorized(chunk_df):
    ...
    grouped = chunk_df.groupby(['contig', 'position','read_name'])
    
    results = []
    for (contig, position, read_name), group in grouped:
...
def merge_duplicate_positions(results):
    ...
    for result in results:
        key = (result['contig'], result['position'], result['read_name'])
...
def merge_multiple_results(results):
    ...
    for result in results:
        for key, value in result.items():
            if key not in ['contig', 'position','read_name']:
...
def main():
    ...
    # Sort by contig, position, and read_name
    result_df = result_df.sort_values(['contig', 'position', 'read_name'])
